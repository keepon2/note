...
├── 直接映射区 (PAGE_OFFSET开始)
│   └── 线性映射物理内存
├── VMALLOC区 (VMALLOC_START开始)
│   └── 动态分配虚拟内存
├── 永久映射区 (PKMAP_BASE开始)
│   └── 永久映射高端内存
├── 固定映射区 (FIXADDR_START开始)
│   └── 固定地址映射
└── 临时映射区 (与固定映射区重叠)
    └── kmap_atomic使用
```
...
| **起始地址** | PAGE_OFFSET |
| **32位地址** | 0xC0000000 |
| **64位地址** | 0xffff880000000000 |
| **映射方式** | 线性映射 |
| **映射关系** | 虚拟地址 = 物理地址 + PAGE_OFFSET |
| **大小** | 896MB或更大 |
| **特点** | 一对一映射，物理内存直接可见，访问速度快 |
| **用途** | 内核数据结构、内核栈、页表等 |
...
| 函数 | 功能 |
|------|------|
| `virt_to_phys(addr)` | 虚拟地址转物理地址 |
| `phys_to_virt(addr)` | 物理地址转虚拟地址 |

**示例：**

| 架构 | 物理地址 | 虚拟地址 |
|------|----------|----------|
| 32位 | 0x00100000 | 0xC0100000 |
| 64位 | 0x00100000 | 0xffff888000100000 |
...
| **起始地址** | VMALLOC_START |
| **结束地址** | VMALLOC_END |
| **32位起始** | 0xC0000000 + 896MB |
| **32位结束** | 0xFEC00000 |
| **64位** | VMALLOC_START ~ VMALLOC_END |
| **映射方式** | 动态非连续映射 |
| **特点** | 非连续物理内存，通过页表映射，动态分配 |
| **访问速度** | 较慢（需要页表查找） |
| **DMA支持** | 不能用于DMA |
...
|------|------|
| **起始地址** | PKMAP_BASE |
| **大小** | PKMAP_SIZE（通常2MB或4MB） |
| **32位地址** | 0xC0000000 + 896MB + 8MB |
| **映射方式** | 固定页表 |
| **页面数量** | LAST_PKMAP个页面 |
| **使用函数** | kmap() |
| **特点** | 固定映射高端内存页面，可以睡眠 |
| **用途** | 访问高端内存，长期映射，需要持久的访问 |
| **限制** | 数量有限，需要互斥锁，用于非原子上下文 |
...
| **32位起始** | 0xFFFFE000 |
| **32位结束** | 0xFFFFFFFF |
| **64位** | FIXADDR_START ~ FIXADDR_TOP |
| **映射方式** | 编译时固定 |
| **特点** | 编译时确定，固定虚拟地址，固定物理地址 |
| **用途** | 固定虚拟地址的物理内存，内核特殊需求，中断处理等 |
| **配置** | 地址固定，不动态分配，编译时配置 |
...
| 函数 | 功能 |
|------|------|
| `fix_to_virt(idx)` | 索引转虚拟地址 |
| `virt_to_fix(addr)` | 虚拟地址转索引 |

**使用场景：**

| 场景 | 说明 |
|------|------|
| 内核特殊需求 | 需要固定虚拟地址的物理内存 |
| 中断处理 | 中断处理中的固定映射 |
| 系统调用 | 系统调用中的固定地址访问 |
...
| **映射方式** | 临时映射 |
| **使用函数** | kmap_atomic() |
| **特点** | 与固定映射区重叠，使用固定映射区的一部分 |
| **映射类型** | 临时映射高端内存，短期访问 |
| **操作类型** | 原子操作，不能睡眠 |
| **用途** | 中断处理，原子上下文，短期访问高端内存 |
| **优势** | 速度快，不需要互斥锁 |
| **限制** | 数量有限 |
...
| 函数 | 功能 |
|------|------|
| `kmap_atomic(page, type)` | 原子映射 |
| `kunmap_atomic(addr)` | 解除原子映射 |
